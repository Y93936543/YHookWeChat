...
添加游戏作弊悬浮球分组
 */
- (void)addSuspendBallSection{
    //获取分组信息
    MMTableViewSectionInfo *sectionInfo = [objc_getClass("MMTableViewSectionInfo") sectionInfoDefaut];
    //将游戏作弊悬浮球cell添加到分组
    [sectionInfo addCell:[self addGameNBSectionCell]];
    //判断是否开起了游戏作弊
    if ([ELAppManage sharedManage].appConfig.SuspendBall)
    {
        //显示游戏作弊悬浮球
        [[YSuspendBall sharedInstance] addToWindow:nil];
    }else{
        [[YSuspendBall sharedInstance] removeFromSuperview];
    }
    //将分组给UITableView
...
/**
 ...
 @return 是否开起游戏作弊cell
 */
- (MMTableViewCellInfo *)addGameNBSectionCell
{
    return [objc_getClass("MMTableViewCellInfo") switchCellForSel:@selector(GameNBClick:) target:self title:@"是否开启游戏作弊" on:[ELAppManage sharedManage].appConfig.SuspendBall];
}

- (void)GameNBClick:(UISwitch *)setpSwitch
{
    [ELAppManage sharedManage].appConfig.SuspendBall = setpSwitch.on;
    [self reloadTableViewData];
}
