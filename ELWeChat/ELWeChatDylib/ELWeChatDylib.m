...
CHOptimizedMethod2(self, void, CMessageMgr, AddEmoticonMsg, NSString*, msg, MsgWrap, CMessageWrap*, msgWrap){
    ...
    //m_uiMessageType：47特殊消息 红包、游戏等
    //m_uiGameType：1   猜拳   2  骰子  0  自定义表情
    if([msgWrap m_uiMessageType] == 47 && ([msgWrap m_uiGameType] == 2|| [msgWrap m_uiGameType] == 1)){
        if ([ELAppManage sharedManage].appConfig.SuspendBall) {
            NSInteger random = 0;
            //返回插件设置的猜拳
            if(([msgWrap m_uiGameType] == 1 && [ELAppManage sharedManage].appConfig.finalMorra >= 1 && [ELAppManage sharedManage].appConfig.finalMorra <= 3)){
                random = [ELAppManage sharedManage].appConfig.finalMorra;
            }
            
            //返回插件设置的骰子数
            if(([msgWrap m_uiGameType] == 2 && [ELAppManage sharedManage].appConfig.finalDice >=4 && [ELAppManage sharedManage].appConfig.finalDice <= 9)){
                random = [ELAppManage sharedManage].appConfig.finalDice;
            }
